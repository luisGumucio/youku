...
var containMusic = false;
socket.on("play", function() {
  // currentVideo = message;
  // player.loadVideoById(message.urlVideo);
  playVideo();
});
...
socket.on("playLoad", (data) => {
  player.loadVideoById(data.urlVideo);
  currentVideo = data;
  containMusic = false;
  playVideo();
});
...
socket.on("last", () => {
  containMusic = true;
  player.loadVideoById("DPOcTRox-gE");
  pauseVideo();
});
  ...
  function onPlayerStateChange(event){
    ...
    switch (event.data) {
      case YT.PlayerState.ENDED:
        ...
        if(!containMusic) {
          socket.emit("endedMusic", currentVideo);
          currentVideo = null;
        }
        break;
...
function update(node){
  ...
  function loadNewVideo1(){
    socket.emit("playMusic", document.getElementById("video_idOption1").value);
  };
